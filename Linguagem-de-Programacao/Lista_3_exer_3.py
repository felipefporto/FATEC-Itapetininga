...
#b) A maior venda do mês 1
    if j == 1:
      maior_1.append(matriz[i][j])
    if j == 2:
      mes_2.append(matriz[i][j])
#c) A menor venda do mês 3
    if j == 3:
      menor_3.append(matriz[i][j])
for i in range(5):
  print('O total vendido pelo', matriz[i][0], 'foi de: R$',sum(soma_venda[i]))
print('A maior venda do mês 1 foi de: ', max(maior_1))
print('A menor venda do mês 3 foi de: ', min(menor_3))
